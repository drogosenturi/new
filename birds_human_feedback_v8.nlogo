...
to birds-reproduce
  ...
  [
    ...
    if reproduce-chance > 0.5 ;; if reproduce change is bigger than 0.5, the reproduce. Less than 50% chance to reproduce.. change?
    [
    hatch 1
    [
      set shape "circle"
...
to update-estimates
  ...
  [
    ...
    if yard-bird-estimate = 0 [ set yard-bird-estimate yard-bird-estimate + 2 ] ;; add 1 b/c nobody estimates 0 birds (SEM data)
...
;        ((max [yard-bird-estimate] of patches) - (min [yard-bird-estimate] of patches)))
    ...
    [ ;ifelse random-float 1 < log-est
      ifelse mean [bird-density] of patches < yard-bird-estimate ;; comapre to mean of all patches, consider changing to median
