...
;; ver 8: logarithmic eq for increasing or decreasing bird-love
...
[
  ...
  veg-change ;; keep track of veg change by patches.. limit?
]
...
to birds-reproduce
  ...
  [
    ...
    let reproduce-chance (random-float 1) / bird-density ;; birds roll random-float one regulated by bird density of the patch
    if reproduce-chance > 0.4 ;; if reproduce change is bigger than 0.5, the reproduce. Less than 50% chance to reproduce.. change?
    [
    hatch 1
    [
      set shape "circle"
...
to update-estimates
  ...
  ask patches
  [
    ;; need less 0's from this equation **** fixed by adding 1 to 0 estimates
    set yard-bird-estimate round ((bird-density + (mean [bird-density] of neighbors)) * (vegetation-volume * ((random-float 1) / 4)))
    if bird-density + mean [bird-density] of neighbors > 0 and yard-bird-estimate = 0
    [ set yard-bird-estimate yard-bird-estimate + 1 ] ;; add 1 b/c nobody estimates 0 birds (SEM data)
...
to change-bird-love ;; adjust this procedure to increase bird love based on birds observed in yard
  ...
  [
    ...
    [ ;ifelse random-float 1 < log-est
      ifelse mean [bird-density] of patches < yard-bird-estimate ;; compare to mean of all patches, consider changing to median
      [ if bird-love < 10 [set bird-love bird-love + 1 set change-count change-count + 1] ] ;; compare how  many time increase vs decrease
      [ if bird-love > 0 [set bird-love bird-love - 1 set change-count change-count + 1] ]
    ]
  ]
  print mean [bird-density] of patches
...
<experiments>
  ...
  <experiment name="sensitivity" repetitions="10" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <final>export-world "world.csv"</final>
